...
[@Cloud/VK Cloud](https://cloud.mail.ru/home/). vCPU = 100% за 740 руб/месяц. Облачная база данных ~1500 руб/месяц.
...
1. Обычные SSL-сертификаты только подтверждают домен. Стоят $18-22/год
...
**UPDATE 2022**: на сайте [ГосУслуги](https://www.gosuslugi.ru/) юридическое лицо может получить бесплатный серверный сертификат, который будет считаться доверенным в браузерах Яндекс.Браузер и VK Atom.

>Три уровня защиты трафика:
>
>- Само-подписанный сертификат на сервер - приводит только к шифрованию трафика, но для подключения к серверу может быть использовано любое ПО, например, Postman. Браузеры не любят само-подписанные сертификаты
>- Двухсторонняя аутентификация - только конкретный клиент может работать с сервером
>- Честные сертификаты - защита и сервера, и клиента от подмены. Очень надёжная схема, но требует профессионального администрирования и управления сертификатами
...
Инфляционное давление привело к критичному росту стоимости оказания услуг. Так, в статье [When Does Inflation Hit Cloud Prices and What Can You Do About It?](https://aws.plainenglish.io/when-does-inflation-hit-cloud-prices-and-what-can-you-do-about-it-ac432f76c5d0) by CAST AI указывается на рост стоимости услуг использования Google Maps в 1400%. Многие из популярных сервисов закрыли бесплатные тарифные планы. Так, например, Heroku прекратила возможность использования тарифного плана Энтузиаст, который был ранее бесплатным.
