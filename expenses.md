...
**ДЛЯ ПРОВЕРКИ**: при открытии слушающего порта в TCP/IP указывается порт и, опционально, IP-адрес. Если IP-адрес не указан, то операционная система слушает локальный порт. Необходимо проверить, что если указать IP-адрес маршрутизатора, то маршрутизатор включит динамическую переадрессацию. Именно таким образом проще всего организовывать peer-to-peer взаимодействие.
