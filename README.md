...
**Update 2024**: в случае использования Expo SDK 50+ рекомендуется обратить внимание на [Expo Router v3](https://docs.expo.dev/router/introduction/). Ещё один интересный Router - [TanStack Router](https://tanstack.com/router/latest).
...
>Одновной тренд современной разработки приложений для мобильных устройств - отказ от инструментов, в которых не обеспечивается Bridgeless. Другими словами, при использовании перехода из платформонезависимой среды (JavaScript) в платформозависимую (Kotlin/Android) происходят критичные потери в производительности решения. Наиболее прогрессимные инструменты пытаются устранить подобные потери разными способами.
...
Операционная система Аврора (ранее Moblin/Meego -> Sailfish) разрабатывается OOO «Открытая мобильная платформа», дочкой РосТелеКома. Базируется на Linux и содержит "Аврора TEE" - дополнительную операционную систему, реализующую функции хранения ключей, по аналогии с ARM TustZone. Пользовательский интерфейс - **Qt Framework**. Display Server - **Wayland**. На [официальном сайте для разработчиков](https://developer.auroraos.ru/) доступен SDK, в котором есть Qt и Qt Creator. Так же следует заметить, что Аврора ОС позволяет использовать Flutter 3.13 и Kotlin Multiplatform (январь 2024 года). Для разработчиков доступен бесплатный курс по разработке приложений на портале Stepik. Также на официальном сайте можно подать заявку на получение устройства на ОС Аврора.
...
Была добавлена статья о [MAUI](./maui.md).

Blazor не совсем корректно относить к разработке мобильных приложений, однако его можно использовать как Progressive Web Apps. В репозитарии, посвещённом web-разработке есть несколько [статей](https://github.com/Kerminator1973/WebDevelopment/blob/master/blazor.md), посвящённых Blazor.

### Обновление 2026 года

В 2024 году Google сильно сократила команду Flutter, а в 2025 году сообщила о прекращении развития и поддержки Flutter. Не смотря на то, что эко-система Flutter очень крепкая, ситуация очень тревожная:

- Члены основной команды уходят:
    - Например, Брэндон ДеРозье, который отвечал за реализацию Flutter GPU (Impeller), указывает в своем LinkedIn, что он покинул команду Flutter в августе 2025 года, чтобы присоединиться к команде Android XR
    - Аналогично, записи о вкладах Джоны Уильямса в GitHub за последние несколько месяцев кажутся в основном неактивными/пустыми.
- Отсутствие коммитов основной команды в основную ветку: если просмотреть коммиты в основной ветке Flutter за последние несколько месяцев, то можно увидеть почти полное отсутствие отправки кода от членов основной команды Flutter. Скорость, кажется, резко упала.

Видимо дальше проект будет развиваться какими-то сторонними командами. Это очень напоминает ситуацию с React Native, который тоже был выделен в отдельную компанию в 2025 году.

## Чем Android принципиально отличается от iOS

Одно из принципиальных отличий - архитектура ядра операционной системы. Android базируется на Linux, тогда как iOS использует **XNU Kernel Architecture**, комбинацию из компонентов the Mach microkernel с компонентами FreeBSD.

The Mach в XNU обрабатывает низко-уровневые задачи, такие как управление папятью, планировщик процессов и межпроцесное взаимодействие (IPC). Уровень BSD реализует традиционное Unix-подобное окружение, включая сетевой уровень (очень эффективно реализованный в FreeBSD), файловую систему и пользовательский интерфейс. Цели объединения:

- высокая производительность гибридного решения
- более высокая гибкость решения при интеграции с различными сервисами и драйверами
...
В 2024 году важной технологией разработки мобильных приложений является [JetPack Compose](https://developer.android.com/develop/ui/compose) - продукт реализует декларативный стиль разработки ПО, в отличии от императивного стиля программирования, доминировавшего в предыдущие годы.
...
**Update 2024 года**: перспективы Flutter/Dart стали ещё менее понятными. С одной стороны, инсталлятор Ubuntu теперь написан на Flutter и Canonical создала отдельную команду для развития Flutter, но с другой стороны - Google сильно сократила команду Flutter/Dart в рамках оптимизации затрат. Отсылая к [статье](https://ln.hixie.ch/?start=1714717681) со-основателя Flutter Иэна Хиксона - количество программистов в core team теперь недостаточно, чтобы сделать Flutter лучшим решением на рынке, но достаточно для того, чтобы продукт развивался (как-то). В корзину голосов "За" - российские разработчики мобильной AuroraOS рассматривают два основных инструмента разработки приложений: Qt и Flutter.

**Update 2026 года**: если руководствоваться материалами статьи [Flutter, Fuchsia, Zircon, ChromeOS, Aluminium OS, WGPU, Tensor Modem, RIL, SeL4, FIDL будущее Андроида и других ОС №2](https://habr.com/ru/articles/984090/) by lil_master, то перспективы Flutter/Dart очень даже интересные. Google изменила стратегию развития Fuchsia, решив не создавать новую операционную систему, а постепенно замещать компоненты Android, решениями из Fuchsia/Zircon. Это является следствием очень высокой инертности рынка. Новые версии Flutter решили многие из технических проблем, связанных с использованием устаревшего API OpenGL. В частности, Flutter перешёл с использования Skia на Impeller, который использует современные API (Metal и Vulkan).

**Update 2025 года**: [банкротство компаний Авроид и Гиперус](https://www.iphones.ru/iNotes/avrora-nikomu-ne-nuzhna-razrabotchiki-prilozheniy-dlya-russkoy-ubiycy-android-okazalis-na-grani-bankrotstva). Первая отвечала за разработку AuroraOS.
...
**Update 2025 года**: корпорация ByteDance выпустила свой собственный framework - Lynx, основная "фишка" которого - использования разных потоков исполнения для UI и фоновых задач. Благодаря этому подходу можно сохранить отзывчивость пользовательского интерфейса даже при выполнении тяжёлых фоновых задач.

## Что происходит в самом начале 2026 годы

Данные взяты из видео "Как устроена Мобильная разработка сегодня?" с канала "Организованное программирование". В качестве гостя приглашён Алексей Гладков.

Фрагментация платформ значительно усилилась. Android - около 25%, iOS - 20%, остальное - китайский сегмент, в котором доминирует Huawei с из операционной системой Harmony OS/Open Harmony, для разработки на которой используется свой собственный язык программирования (похож на TypeScript) и свой собственный SDK. 

Китайские производители мобильных телефонов используют в качестве базы как AOSP, так и Harmony OS, добавляя свой собственный UI. Часто для западных рынков устанавливается AOSP, а для aзиатских и aфриканских - Harmony.

Из-за высокой фрагментации нативная разработка, фактически, мертва. Есть огромное количество legacy-приложений, которые ещё сопровождают, но новые решения делать на нативных инструментах крайно дорого - это проигрышная бизнес-стратегия. 

Азиатские разработчики предпочитают использовать кросс-платформенные инструменты, а не нативные. Популярны:

- **BDUI (Backend-Driven UI)**, он же SDUI (Server-Driven UI). В частности, у Яндекса есть opensource фреймворк **DivKit** и **Flex SDK**
- Flutter. Dart - скорее негативный фактор, чем позитивный 
- React Native. Expo похожа на _killer feature_
- Kotlin Multiplatform (вместе с JetPack Compose)
- PWA (Progressive Web App)

Выигрывают BDUI.

Swift потихонечку также становится мультиплатформенным.

Часть разработчиков JetBrains, которые не уехали за рубеж в 2022 году, перешли в Huawei и работают над инструментами Open Harmony.

Flutter и Compose Multiplatform, являясь разработками Google, являются прямыми конкурентами. Пока Flutter побеждает за счёт лучшей документации, поддержки сообществом, но Google больше поддерживает Compose. Flutter - это система со своим языком, тогда как Compose Multiplatform использует универсальные инструменты, в частности - Kotlin.

### Что такое BDUI

BDUI — это подход в мобильной разработке, при котором структура и содержимое пользовательского интерфейса формируются на стороне сервера, а мобильное приложение лишь отображает то, что ему прислали.

Как это работает:

- Сервер описывает интерфейс. Бэкенд возвращает клиенту описание экрана — обычно в формате JSON. В нём перечислены компоненты (кнопки, карточки, списки и т. п.), их порядок, параметры и данные для отображения
= Клиент рендерит интерфейс. В мобильном приложении есть "движок рендеринга": он читает эту схему и собирает нативный UI из заранее поддерживаемых компонентов.
- Изменения идут через сервер. Чтобы поменять экран (например, добавить блок или изменить порядок секций), достаточно обновить данные на бэкенде — новую версию приложения выпускать не нужно

Достоинства подхода:

- Быстрые изменения без релиза. Можно менять экраны, не дожидаясь подтверждения (approve) в App Store или Google Play
- Персонализация. Разные пользователи могут видеть разные варианты интерфейса (например, в зависимости от сегмента, региона или поведения)
- A/B‑тесты и эксперименты. Легко тестировать разные макеты и быстро откатывать неудачные варианты
= Унификация между платформами. Одна схема с сервера может использоваться для iOS, Android и веба — меньше дублирования логики

## Что происходит в середине 2026 года

Началась "эпоха дешевого кода"! Теперь можно дешево писать нативный код на Swift/Kotlin/Java. React Native является дополнительным слоем, что создаёт ряд неудобств:

- дополнительный технологический stack
- прослойка, которая требует вычислительных ресурсов
- функциональность отстаёт от нативного кода (особенно в части работы с Hardware)
- есть целый ряд особенностей, который следует учитывать при разработке кода (state management, memoization)

Т.е. теперь React native (Flutter, и т.д.) это уже не сокращение расходов, а скорее ограничения. Это значит, что будущее таких инструментов становится очень неопределённым. Отчасти это объясняет, почему Google и Facebook сократили продуктовые команды Flutter и React Native, а затем вывели эти проекты за пределы своих организационных структур.

В зоне риска оказываются также Avalonia и MAUI.
