...
Я пока ещё не добрался до [Flutter](https://flutter.dev/) и [Dart](https://dart.dev/). Google **Fuchsia** - микроядерная операционная система для мобильных устройств. Основной язык разработки приложений - Dart (с Flutter). Ядро написано, преимущественно, на Rust. Проект переведён в open-source, перспективы не очень понятеы.
...
Не смотря на то, что со Swift можно попрактиковаться в [Swift Playgrounds](https://apps.apple.com/ru/app/swift-playgrounds/id908519492) на iPad, Xcode я пробовал только на *Хакинтош*, что нельзя назвать "релевантным опытом". С переходом на Apple Silicon (M1) эмулятор iOS работает на той же аппаратной платформе, что и настоящий iOS.
