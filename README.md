...
Попрактиковаться в разработке приложений под iOS можно на оборудовании Apple, например, на Mac Mini. Альтернативой может быть использование *Хакинтош*, но это не официальный путь, сопряжённый со множеством ограничений, включая использование старых образов ОС, потерю производительности из-за виртуализации, и т.д. Мой опыт использования Xcode на "Хакинтош", скорее негативный. Также может быть использован [Swift Playgrounds](https://apps.apple.com/ru/app/swift-playgrounds/id908519492) на iPad. в середине 2021 года Playground предоставлял возможность работать с единственным файлом в проекте и создавать приложение с использованием Swift UI, с обеспечением полноценного Preview. Обойти ограничение в один файл можно использую дополнительный инструментарий (например, платный Textastic Code Editor и Working Copy Git Client). В iPadOS 15 возможностей у Swift Playground должно стать существенно больше, включая публикацию разработанного приложения в Apple AppStore. Также, см. Snack выше по тексту.

Пока не очень понятно, как развивает свою эко-систему [Huawei](https://developer.huawei.com/consumer/en/doc/30114).

Ссылки на [Ionic](https://ionicframework.com/) и [Apache Cordova](https://cordova.apache.org/) встречаются достаточно часто, но не настолько часто, чтобы его можно было рассматривать как конкурента React Native, или Flutter.
